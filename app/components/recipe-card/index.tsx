...
const actionClass =
  'inline-flex items-center p-3 rounded-md text-gray-800 bg-gray-300 dark:text-gray-200 dark:bg-slate-700'
      ...
      <div className="flex flex-col items-start h-full flex-1 p-4 gap-5">
        ...
        <div className="flex w-full gap-3">
          {preview ? (
            ...
            <a className={actionClass} href={url} aria-label="Open Recipe">
              <FiExternalLink />
            </a>
          )}

          <button className={actionClass} aria-label={`Edit ${name}`} onClick={edit} disabled={preview}>
            <FiEdit />
          </button>

          <button className={actionClass} aria-label={`Share ${name}`} onClick={share} disabled={preview}>
            <FiShare2 />
          </button>
        </div>
      </div>
