...
type Partitions = {
	red: ConfigIndicator;
	green: ConfigIndicator;
	blue: ConfigIndicator;
};
...
export function changeCaretColor(e: MouseEvent | TouchEvent) {
	...
	if ("offsetX" in e) {
		...
		x =
			e.touches?.length && e.touches[0].clientX >= 0 ? e.touches[0].clientX : 0;
	}
	...
	for (const caret of document.querySelectorAll(".caret")) {
